...
import { RATE_LIMITS, perIpRateLimit } from '../lib/rateLimits.js';
...
export const exportRoutes: FastifyPluginAsync<{ exporter: ExportService }> = async (
  ...
  // A stricter budget than the default public-read limit: each call can stream up to 50,000 rows.
  app.get(
    '/api/export.csv',
    { config: perIpRateLimit(RATE_LIMITS.export) },
    async (request, reply) => {
