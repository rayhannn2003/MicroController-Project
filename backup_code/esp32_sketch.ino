...
// WebServer and esp_http_server define different HTTP_ANY macros.
// Every route below explicitly uses HTTP_GET, so neither wildcard is needed.
#undef HTTP_ANY
...
void handleStatus()
{
    ...
    json += ",\"streamReady\":";
    json += streamReady ? "true" : "false";
    json += ",\"streamPort\":";
    json += String(STREAM_PORT);
    json += "}";
...
esp_err_t captureJpeg(uint8_t **jpgBuffer, size_t *jpgLength, uint8_t quality,
                      bool freshFrame)
{
    *jpgBuffer = nullptr;
    *jpgLength = 0;
    ...
    // Only one task may acquire a raw frame and run the software JPEG encoder
    // at a time. Release the lock before any potentially slow network writes.
    if (xSemaphoreTake(cameraMutex, pdMS_TO_TICKS(2000)) != pdTRUE)
        return ESP_ERR_TIMEOUT;

    camera_fb_t *fb = esp_camera_fb_get();
    if (fb != nullptr && freshFrame)
    {
        // WHEN_EMPTY can retain an old frame while streaming is stopped.
        // Discard it so a snapshot shows the scene after the button was pressed.
    ...
    bool success = frame2jpg(fb, quality, jpgBuffer, jpgLength);
    esp_camera_fb_return(fb);
    xSemaphoreGive(cameraMutex);

    if (!success || *jpgBuffer == nullptr || *jpgLength == 0)
    {
        free(*jpgBuffer);
        *jpgBuffer = nullptr;
        *jpgLength = 0;
    ...
    // The caller owns this JPEG allocation and must free it on every path.
    return ESP_OK;
}


// =====================================================
// CAPTURE IMAGE (port 80)
// =====================================================
...
void handleCapture()
{
    ...
    if (!cameraReady)
    {
        server.send(503, "text/plain", "Camera unavailable. Check the board and restart it.");
        return;
    }

    uint8_t *jpgBuffer = nullptr;
    size_t jpgLength = 0;
    esp_err_t result = captureJpeg(&jpgBuffer, &jpgLength, CAPTURE_JPEG_QUALITY, true);

    if (result != ESP_OK)
    {
        server.send(result == ESP_ERR_TIMEOUT ? 503 : 500, "text/plain",
                    result == ESP_ERR_TIMEOUT ? "Camera busy. Please try again."
                                              : "Camera capture or JPEG conversion failed.");
        return;
    }

    server.sendHeader("Content-Disposition", "inline; filename=sylvan-capture.jpg");
    server.setContentLength(jpgLength);
    server.send(200, "image/jpeg", "");

    WiFiClient client = server.client();
    size_t sent = 0;
    while (sent < jpgLength && client.connected())
    {
        size_t remaining = jpgLength - sent;
        size_t written = client.write(jpgBuffer + sent, remaining > 1024 ? 1024 : remaining);
        if (written == 0) break;
        sent += written;
        readAtmegaUART();
        delay(1);
    ...
    {
        client.stop();
        Serial.println("Snapshot client disconnected before the image was sent.");
    }
    else
    {
...
// =====================================================
// MJPEG LIVE STREAM (port 81, separate HTTP server task)
// =====================================================
...
esp_err_t handleStream(httpd_req_t *request)
{
    if (!cameraReady)
    {
        ...
        return httpd_resp_sendstr(request, "Camera unavailable");
    }

    httpd_resp_set_type(request, "multipart/x-mixed-replace;boundary=sylvanframe");
    ...
    while (true)
    {
        ...
        uint8_t *jpgBuffer = nullptr;
        size_t jpgLength = 0;
        esp_err_t result = captureJpeg(&jpgBuffer, &jpgLength, STREAM_JPEG_QUALITY, false);

        if (result != ESP_OK)
        {
            Serial.printf("Stream capture failed: 0x%x\n", result);
            if (!sentFrame)
                return httpd_resp_send_err(request, HTTPD_500_INTERNAL_SERVER_ERROR,
                                           "Camera capture or JPEG conversion failed");
            // Headers have already been sent. Close this stream so it can be restarted.
            return ESP_FAIL;
        }

        char partHeader[96];
        int headerLength = snprintf(partHeader, sizeof(partHeader),
            "\r\n--sylvanframe\r\nContent-Type: image/jpeg\r\nContent-Length: %u\r\n\r\n",
            (unsigned int)jpgLength);

        result = httpd_resp_send_chunk(request, partHeader, headerLength);
        if (result == ESP_OK)
            result = httpd_resp_send_chunk(request, (const char *)jpgBuffer, jpgLength);

        free(jpgBuffer);
        if (result != ESP_OK)
        {
            // Browser stopped streaming, disconnected or exceeded the send timeout.
            return result;
        }
        sentFrame = true;

        // RGB565 -> JPEG is CPU-intensive on GC2145. Pace frames and yield to
        // Wi-Fi/UART tasks; retain one raw framebuffer even when PSRAM is present.
        unsigned long elapsed = millis() - frameStart;
        delay(elapsed < STREAM_FRAME_INTERVAL_MS ? STREAM_FRAME_INTERVAL_MS - elapsed : 1);
    }
}
...
bool startStreamServer()
{
    ...
    esp_err_t result = httpd_start(&streamServer, &config);
    if (result != ESP_OK)
    {
    ...
    streamRoute.handler = handleStream;
    result = httpd_register_uri_handler(streamServer, &streamRoute);
    if (result != ESP_OK)
    {
        httpd_stop(streamServer);
        streamServer = nullptr;
        Serial.printf("Stream route registration failed: 0x%x\n", result);
        return false;
    }
...
void setup()
{
    ...
    */
    ...
    cameraReady = cameraMutex != nullptr && initCamera();
    if (!cameraReady)
    {
        Serial.println("Camera unavailable. Portal and sensor monitoring will still start.");
    }
    ...
    streamReady = cameraReady && startStreamServer();
    server.begin();

    if (streamReady)
    {
        Serial.printf("Live stream: http://%s:%u/stream\n", IP.toString().c_str(), STREAM_PORT);
    }
    Serial.printf("Still image: http://%s/capture\n", IP.toString().c_str());
